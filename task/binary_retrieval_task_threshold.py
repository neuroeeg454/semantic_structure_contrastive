...
class BinaryRetrievalTaskThreshold:
    """
    Binary Retrieval Task (Threshold Version)
    """
    def __init__(self, config: Dict[str, Any], device: str = None):
        ...
        self.model = None
        self.criterion = None
        self.exp_dir = Path(config.get('exp_dir', 'experiments/binary_retrieval_threshold'))

        self.logger = logging.getLogger(__name__)
        model_config = self.config.get('model', {}).get('contrastive', {})
        temperature = float(model_config.get('temperature', 0.07))
        
        # Get semantic threshold from config, default to 0.8
        semantic_threshold = float(model_config.get('semantic_threshold', 0.8))
        
        # Enable use_semantic_positives
        self.criterion = ContrastiveLoss(
            temperature=temperature,
            use_semantic_positives=True,
            semantic_threshold=semantic_threshold
        ).to(self.device)


    def _extract_pos_neg_semantic(self, similarity_matrix: torch.Tensor, text_embeddings: torch.Tensor, threshold: float = 0.8) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        batch_size = similarity_matrix.size(0)
        device = similarity_matrix.device
        text_norm = F.normalize(text_embeddings, p=2, dim=-1)
        text_sim_matrix = torch.matmul(text_norm, text_norm.T) # [batch, batch]
        labels_matrix = (text_sim_matrix > threshold).long()
        pos_mask = (labels_matrix == 1)
        pos_sim = similarity_matrix[pos_mask]
        neg_mask = (labels_matrix == 0)
    ...
    def compute_metrics(self, eeg_embeddings: torch.Tensor, text_embeddings: torch.Tensor) -> Dict[str, float]:
        with torch.no_grad():
            ...
            pos_sim_all, _, mask, labels_matrix = self._extract_pos_neg_semantic(logits, text_embeddings)
            
            # ==============================
            # Binary Accuracy (Pairwise)
            # ==============================
            # Calculate accuracy by comparing ALL positive pairs vs ALL negative pairs for each query
            
            # EEG -> Text
            acc_list = []
            for i in range(batch_size):
                row_labels = labels_matrix[i].bool()
                pos_scores = logits[i][row_labels]
                neg_scores = logits[i][~row_labels]
                
                if len(pos_scores) > 0 and len(neg_scores) > 0:
                    # Compare every positive score with every negative score
                    # [P, 1] > [1, N] -> [P, N]
                    compare = (pos_scores.unsqueeze(1) > neg_scores.unsqueeze(0)).float()
                    acc_list.append(compare.mean())
            
            binary_acc_eeg = torch.stack(acc_list).mean() if acc_list else torch.tensor(1.0, device=self.device)

            # Text -> EEG
            acc_list_t = []
            logits_t = logits.T
            # Note: labels_matrix is symmetric because text_sim_matrix is symmetric
            for i in range(batch_size):
                col_labels = labels_matrix[i].bool()
                pos_scores = logits_t[i][col_labels]
                neg_scores = logits_t[i][~col_labels]
                
                if len(pos_scores) > 0 and len(neg_scores) > 0:
                    compare = (pos_scores.unsqueeze(1) > neg_scores.unsqueeze(0)).float()
                    acc_list_t.append(compare.mean())

            binary_acc_text = torch.stack(acc_list_t).mean() if acc_list_t else torch.tensor(1.0, device=self.device)
    ...
    def evaluate_on_dataloader(self, dataloader, return_embeddings: bool = False, return_predictions: bool = False, **kwargs) -> Dict[str, Any]:
        ...
        with torch.no_grad():
            pbar = tqdm(dataloader, desc="binary retrieval (Threshold)")
    ...
    def run_analysis(self, test_results: Dict[str, Any], top_n: int = 20):
        ...
        with torch.no_grad():
            ...
            pos_similarities, neg_similarities, _, _ = self._extract_pos_neg_semantic(similarity, text_embeddings)
